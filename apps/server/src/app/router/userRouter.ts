...
export const userRouter = router({
  register: procedure.input(registerUserUsecase.inputSchema).mutation(async ({ input, ctx }) => {
    return registerUserUsecase.run(ctx, input)
  }),

  login: procedure.input(loginUserUsecase.inputSchema).mutation(async ({ input, ctx }) => {
